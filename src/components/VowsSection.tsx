...
const VowsSection = () => {
  ...
  return (
    <section id="vows" className="snap-section grain relative py-32 md:py-40" style={{ backgroundColor: "hsl(var(--vows-bg))", isolation: "isolate" }}>
      ...
      <div ref={ref} className="relative z-10 max-w-[900px] mr-auto ml-0 px-3 text-left">
        <h3
          ...
          style={{ ...revealStyle(isVisible, 0), color: c.color_title || "hsl(var(--vows-title))" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {c.cards.map((vow, i) => (
            <div
              ...
              <p className="font-body-heading text-sm font-bold mb-3" style={{ color: c.color_card_title || "hsl(var(--vows-card-title))" }}>{vow.title}</p>
              <div className="font-body text-xs leading-relaxed" style={{ color: c.color_card_body || "hsl(var(--vows-card-body))", overflow: "visible", height: "auto" }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(vow.body) }} />
            </div>
          ))}
        </div>
      </div>
