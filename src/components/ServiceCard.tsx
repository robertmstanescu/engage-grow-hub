  ...
  return (
    <motion.div
      ...
      <div className="p-5 md:p-6">
        ...
        <p className="font-body text-xs leading-relaxed line-clamp-3" style={{ color: "var(--pillar-card-description, hsl(var(--foreground) / 0.65))" }}>{description}</p>
      </div>
      ...
      <div className="px-5 md:px-6 py-3 flex justify-between items-center flex-wrap gap-2" style={{ backgroundColor: "hsl(var(--background) / 0.3)" }}>
        <a href="#contact" className="font-display text-[11px] font-bold tracking-wide hover:opacity-80 transition-all duration-500" style={{ color: "var(--pillar-cta-text, hsl(var(--accent)))" }}>{price} →</a>
        <span className="font-body text-[11px] tracking-wide" style={{ color: "var(--pillar-cta-time, hsl(var(--foreground) / 0.4))" }}>{time}</span>
      </div>
      {note && (
