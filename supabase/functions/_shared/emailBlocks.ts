/**
 * Canonical email-campaign block model + renderer.
 *
 * This is the SINGLE source of truth for turning an admin's block-based
 * campaign editor content into the HTML that both the admin preview and
 * the actual sent email use. It used to be duplicated — once in
 * src/features/admin/email-blocks.ts (browser, drives the live iframe
 * preview) and once inline in supabase/functions/send-campaign/index.ts
 * (Deno, drives the actual send) — and the two copies drifted out of
 * sync (missing blocks, different padding, different fonts) because
 * there was no single place to change. Both now import from here:
 *   - src/features/admin/email-blocks.ts re-exports this module for the
 *     browser bundle.
 *   - supabase/functions/send-campaign/index.ts imports it directly via
 *     a relative path (Deno edge functions share code via `_shared/`
 *     folders — see the sibling `_shared/email-templates` /
 *     `_shared/transactional-email-templates` for the same pattern).
 *
 * Plain TS, no browser or Deno-specific APIs, so it runs unmodified in
 * both runtimes.
 */
        ...
        return `<tr><td style="padding:24px 20px;background:#ffffff;">
          <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #e8e0d8;border-radius:12px;overflow:hidden;">
            ...
            <tr><td style="padding:24px;">
              ...
              <h2 style="font-family:'Bricolage Grotesque',sans-serif;font-size:18px;color:#2A0E33;margin:0 0 8px 0;line-height:1.3;">${block.settings.blogTitle || ""}</h2>
              ...
              <a href="${block.settings.blogUrl || "#"}" style="display:inline-block;background:#4D1B5E;color:#F9F0C1;padding:10px 24px;border-radius:50px;text-decoration:none;font-family:'Bricolage Grotesque',sans-serif;font-size:11px;font-weight:bold;text-transform:uppercase;letter-spacing:0.08em;">Read Article</a>
            </td></tr>
          </table>
        </td></tr>`;

      case "button":
        return `<tr><td style="padding:${block.settings.padding || "24px 20px"};background:${block.settings.backgroundColor || "#ffffff"};text-align:${block.settings.alignment || "center"};"><a href="${block.settings.buttonUrl || "#"}" style="display:inline-block;background:${block.settings.buttonBg || "#4D1B5E"};color:${block.settings.buttonColor || "#F9F0C1"};padding:12px 28px;border-radius:50px;text-decoration:none;font-family:'Bricolage Grotesque',sans-serif;font-size:12px;font-weight:bold;text-transform:uppercase;letter-spacing:0.08em;">${block.settings.buttonText || "Click Here"}</a></td></tr>`;
